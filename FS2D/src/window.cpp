...
MainWindow::MainWindow(sf::VideoMode _videoMode, const sf::String& _title, sf::Uint32 _style,
                       const sf::ContextSettings& _ctxSettings)
    : sf::RenderWindow(_videoMode, _title, _style, _ctxSettings), mBaseProperties(_videoMode.width, _videoMode.height)
{
    mPixelBuffer.resize(static_cast<size_t>(mBaseProperties.fieldWidth * mBaseProperties.fieldHeight * 4));
    mFluidTexture.create(mBaseProperties.fieldWidth, mBaseProperties.fieldHeight);
...
void MainWindow::mainLoop()
{
    ...
    while (isOpen())
    {
        ...
        if (!bIsPaused)
        {
            ...
            compute(mPixelBuffer.data(), mLastMousePos.x, mLastMousePos.y, mCurMousePos.x, mCurMousePos.y,
                    bIsInfluenced);
        }
...
{
    setConfig(mFluidConfig.velocityDiffusion, mFluidConfig.colorDiffusion, mFluidConfig.densityDiffusion,
              mFluidConfig.pressure, mFluidConfig.vorticity, mFluidConfig.forceScale, mFluidConfig.bloomIntense,
              mFluidConfig.dt, mFluidConfig.radius, mFluidConfig.velocityIterations, mFluidConfig.pressureIterations,
              mFluidConfig.xThreads, mFluidConfig.yThreads, mFluidConfig.bloomEnabled, mFluidConfig.bColorful,
              mFluidConfig.bParallel, mFluidConfig.color[0], mFluidConfig.color[1], mFluidConfig.color[2]);
}
