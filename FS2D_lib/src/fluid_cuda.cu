...
__device__ Coord2D computeU(Particle* _particleField, size_t _xSize, size_t _ySize, const Coord2DI& _coordV,
                            const Coord2D& _coordB, float _alpha, float _beta)
{
    int posX = _coordV.x, posY = _coordV.y;
    auto f = [&](int _x, int _y)
    { return checkBounds(_x, _y, _xSize, _ySize) ? _particleField[_y * _xSize + _x].u : -_coordB; };
    return (f(posX, posY - 1) + f(posX, posY + 1) + f(posX + 1, posY) + f(posX - 1, posY) + _coordB * _alpha) *
           (1.0f / _beta);
}

__device__ float computeP(float* _pressureField, size_t _xSize, size_t _ySize, const Coord2DI& _coord, float _B,
                          float _alpha, float _beta)
{
    int posX = _coord.x, posY = _coord.y;
    auto f = [&](int x, int y) {
        return checkBounds(x, y, _xSize, _ySize) ? _pressureField[y * _xSize + x]
                                                 : -_pressureField[posY * _xSize + posX];
    };
    return (f(posX, posY - 1) + f(posX, posY + 1) + f(posX - 1, posY) + f(posX + 1, posY) + _alpha * _B) *
           (1.0f / _beta);
}

__device__ RGBColor computeC(Particle* _colorField, size_t _xSize, size_t _ySize, const Coord2DI& _coordPos,
                             const RGBColor& _colorB, float _alpha, float _beta)
{
    int x = _coordPos.x, y = _coordPos.y;
    auto f = [&](int _x, int _y)
    { return checkBounds(_x, _y, _xSize, _ySize) ? _colorField[_y * _xSize + _x].color : RGBColor(); };
...
{
    int x1 = sci(_pos.x), y1 = sci(_pos.y), x2 = sci(_pos.x) + 1, y2 = sci(_pos.y) + 1;
    ...
    float t1 = (x2 - _pos.x) / (x2 - x1), t2 = (_pos.x - x1) / (x2 - x1), t3 = (y2 - _pos.y) / (y2 - y1),
          t4 = (_pos.y - y1) / (y2 - y1);
...
__device__ float divergency(Particle* _particleField, size_t _xSize, size_t _ySize, const Coord2DI& _coordPos)
{
    ...
    auto f = [&](int _x, int _y)
    { return checkBounds(_x, _y, _xSize, _ySize) ? _particleField[_y * _xSize + _x].u.x : -C.u.x; };
    auto g = [&](int _x, int _y)
    { return checkBounds(_x, _y, _xSize, _ySize) ? _particleField[_y * _xSize + _x].u.y : -C.u.y; };
...
__device__ Coord2D gradient(float* _field, size_t _xSize, size_t _ySize, const Coord2DI& _coordPos)
{
    int posX = _coordPos.x, posY = _coordPos.y;
    auto f = [&](int _x, int _y)
    { return checkBounds(_x, _y, _xSize, _ySize) ? _field[_y * _xSize + _x] : _field[posY * _xSize + posX]; };
...
__device__ Coord2D absGradient(float* _field, size_t _xSize, size_t _ySize, const Coord2DI& _coordPos)
{
    ...
    auto f = [&](int _x, int _y)
    { return checkBounds(_x, _y, _xSize, _ySize) ? _field[_y * _xSize + _x] : _field[posY * _xSize + posX]; };
    return { (abs(f(posX + 1, posY)) - abs(f(posX - 1, posY))) / 2.0f,
             (abs(f(posX, posY + 1)) - abs(f(posX, posY - 1))) / 2.0f };
}
...
__device__ float curl(Particle* _particleField, size_t _xSize, size_t _ySize, const Coord2DI& _coordPos)
{
    ...
    auto f = [&](int _x, int _y)
    {
        return checkBounds(_x, _y, _xSize, _ySize) ? _particleField[_y * _xSize + _x].u.x
                                                   : -_particleField[posY * _xSize + posX].u.x;
    };
    auto g = [&](int _x, int _y)
    {
        return checkBounds(_x, _y, _xSize, _ySize) ? _particleField[_y * _xSize + _x].u.y
                                                   : -_particleField[posY * _xSize + posX].u.y;
    };
    return ((g(posX, posY + 1) - g(posX, posY - 1)) - (f(posX + 1, posY) - f(posX - 1, posY))) / 2.0f;
}

__global__ void advect(Particle* _particleCurrField, Particle* _particlePrevField, size_t _xSize, size_t _ySize,
                       float _densityDiffusion, float _dt)
{
    ...
    _particleCurrField[y * _xSize + x] = {
        p.u * decay, { pow(p.color.R, 1.005f) * decay, pow(p.color.G, 1.005f) * decay, pow(p.color.B, 1.005f) * decay }
    };
}
...
{
    int x = blockIdx.x * blockDim.x + threadIdx.x, y = blockIdx.y * blockDim.y + threadIdx.y;
    float R = _particleField[y * _xSize + x].color.R, G = _particleField[y * _xSize + x].color.G,
          B = _particleField[y * _xSize + x].color.B;
...
__global__ void computeVelocity(Particle* _particleCurrField, Particle* _particlePrevField, size_t _xSize,
                                size_t _ySize, float _velocityDiffusion, float _dt)
{
    int x = blockIdx.x * blockDim.x + threadIdx.x, y = blockIdx.y * blockDim.y + threadIdx.y;
    float alpha = _velocityDiffusion * _velocityDiffusion / _dt;
    _particleCurrField[y * _xSize + x].u = computeU(_particlePrevField, _xSize, _ySize, { x, y },
                                                    _particlePrevField[y * _xSize + x].u, alpha, 4.0f + alpha);
}

__global__ void computePressure(Particle* _particleCurrField, size_t _xSize, size_t _ySize, float* _currP,
                                float* _prevP, float _pressure)
{
    int x = blockIdx.x * blockDim.x + threadIdx.x, y = blockIdx.y * blockDim.y + threadIdx.y;
    _currP[y * _xSize + x] =
        computeP(_prevP, _xSize, _ySize, { x, y }, divergency(_particleCurrField, _xSize, _ySize, { x, y }),
                 -_pressure * _pressure, 4.0f);
}

__global__ void computeColor(Particle* _particleCurrField, Particle* _particlePrevField, size_t _xSize, size_t _ySize,
                             float _colorDiffusion, float _dt)
{
    int x = blockIdx.x * blockDim.x + threadIdx.x, y = blockIdx.y * blockDim.y + threadIdx.y;
    float alpha = _colorDiffusion * _colorDiffusion / _dt;
    _particleCurrField[y * _xSize + x].color = computeC(_particlePrevField, _xSize, _ySize, { x, y },
                                                        _particlePrevField[y * _xSize + x].color, alpha, 4.0f + alpha);
}
...
__global__ void applyForce(Particle* _particleField, size_t _xSize, size_t _ySize, RGBColor _color, Coord2D _coordF,
                           Coord2DI _pos, int _r, float _dt)
{
...
__global__ void applyVorticity(Particle* _particleCurrField, Particle* _particlePrevField, float* _fieldV,
                               size_t _xSize, size_t _ySize, float _vorticity, float _dt)
{
    int x = blockIdx.x * blockDim.x + threadIdx.x, y = blockIdx.y * blockDim.y + threadIdx.y;
...
__global__ void applyBloom(uint8_t* _colorField, size_t _xSize, Coord2DI _pos, int _radius, float _bloomIntense)
{
    int x = blockIdx.x * blockDim.x + threadIdx.x, y = blockIdx.y * blockDim.y + threadIdx.y,
        pos = 4 * (y * _xSize + x);
    float e =
        _bloomIntense * expf(-((x - _pos.x) * (x - _pos.x) + (y - _pos.y) * (y - _pos.y) + 1.0f) / (_radius * _radius));
...
// Export functions
void setConfig(float _velocityDiffusion, float _colorDiffusion, float _densityDiffusion, float _pressure,
               float _vorticity, float _forceScale, float _bloomIntense, float _dt, int _radius,
               int _velocityIterations, int _pressureIterations, int _xThreads, int _yThreads, bool _bloomEnabled,
               bool _colorful, bool _parallel, unsigned char _r, unsigned char _g, unsigned char _b)
{
    ...
    globalColorArray[0] =
        globalConfig.bColorful
            ? RGBColor({ 1.0f, 0.0f, 0.0f })
            : RGBColor({ globalConfig.color[0] / 255.f, globalConfig.color[1] / 255.f, globalConfig.color[2] / 255.f });
}
...
{
    dim3 threadsPerBlock(globalConfig.bParallel ? globalConfig.xThreads : 1,
                         globalConfig.bParallel ? globalConfig.yThreads : 1);
    ...
    applyVorticity<<<numBlocks, threadsPerBlock>>>(globalCurrField, globalPrevField, globalVorticityField, globalXSize,
                                                   globalYSize, globalConfig.vorticity, globalConfig.dt);
    std::swap(globalPrevField, globalCurrField);

    for (int i = 0; i < globalConfig.velocityIterations; ++i)
    {
        computeVelocity<<<numBlocks, threadsPerBlock>>>(globalCurrField, globalPrevField, globalXSize, globalYSize,
                                                        globalConfig.velocityDiffusion, globalConfig.dt);
        computeColor<<<numBlocks, threadsPerBlock>>>(globalCurrField, globalPrevField, globalXSize, globalYSize,
                                                     globalConfig.colorDiffusion, globalConfig.dt);
    ...
    if (_isPressed)
    {
        ...
        globalCurrentColor = globalConfig.bColorful
                                 ? globalColorArray[sci(elapsedTime) % globalColorArraySize] * (1 - w) +
                                       globalColorArray[sci((elapsedTime) + 1) % globalColorArraySize] * w
                                 : globalColorArray[0];
        applyForce<<<numBlocks, threadsPerBlock>>>(
            globalPrevField, globalXSize, globalYSize, globalCurrentColor,
            { (_currX - _prevX) * globalConfig.forceScale, (_currY - _prevY) * globalConfig.forceScale },
            { _currX, _currY }, globalConfig.radius, globalConfig.dt);
    }
    else
    {
    ...
    {
        computePressure<<<numBlocks, threadsPerBlock>>>(globalPrevField, globalXSize, globalYSize, globalCurrPressure,
                                                        globalPrevPressure, globalConfig.pressure);
    ...
    advect<<<numBlocks, threadsPerBlock>>>(globalCurrField, globalPrevField, globalXSize, globalYSize,
                                           globalConfig.densityDiffusion, globalConfig.dt);
    std::swap(globalCurrField, globalPrevField);
    ...
    {
        applyBloom<<<numBlocks, threadsPerBlock>>>(globalColorField, globalXSize, { _currX, _currY },
                                                   globalConfig.radius, globalConfig.bloomIntense);
    }
